...
public class RarUtil {
    ...
    private static final Logger LOG = Logger.getLogger(RarUtil.class.getName());
    ...
    public static void extractArchive(File archive, File destination) {
        ...
        if (arch != null) {
            if (arch.isEncrypted()) {
                LOG.log(Level.WARNING, "archive is encrypted cannot extreact");
                return;
            }
            FileHeader fh = null;
            while (true) {
                ...
                if (fh.isEncrypted()) {
                    LOG.log(Level.WARNING, "file is encrypted cannot extract: "
                            + fh.getFileNameString());
                    continue;
                }
                LOG.log(Level.INFO, "extracting: " + fh.getFileNameString());
                try {
                    if (fh.isDirectory()) {
                    ...
                    LOG.log(Level.SEVERE, "error extracting the file", e);
                }
            }
        }
    }
    ...
    private static File createFile(FileHeader fh, File destination) {
        ...
        if (!f.exists()) {
            try {
                ...
                LOG.log(Level.SEVERE, "error creating the new file: " + f.getName(), e);
            }
        }
        return f;
    }
